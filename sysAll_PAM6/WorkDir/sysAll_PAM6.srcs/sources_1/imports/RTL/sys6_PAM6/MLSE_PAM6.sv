...
module MLSE_PAM6 #(
    ...
    // debug start - Dump y0 to y5 at simulation start 
    integer m, n;
    initial begin
        $display("=== MLSE y0-y5 dump ===");
        for (m = 0; m < 6; m = m + 1) begin
            $write("%0d: ", m);
            case (m)
                0: for (n = 0; n < 6; n = n + 1) $write("%0d ", y0[n]);
                1: for (n = 0; n < 6; n = n + 1) $write("%0d ", y1[n]);
                2: for (n = 0; n < 6; n = n + 1) $write("%0d ", y2[n]);
                3: for (n = 0; n < 6; n = n + 1) $write("%0d ", y3[n]);
                4: for (n = 0; n < 6; n = n + 1) $write("%0d ", y4[n]);
                5: for (n = 0; n < 6; n = n + 1) $write("%0d ", y5[n]);
            endcase
            $display("");
        end
        $display("=======================");
    ...
    always @(posedge clk) begin
        if (!rstn) begin
            ...
            // debug start - Inside always @(posedge clk), after updating survivor_path_metrics and prev_survivor_path_state ---
            if (step_count < 11) begin
                $display("Step %0d (delay %0d): signal_in=%0d, survivor_metrics=[%0d, %0d, %0d, %0d, %0d, %0d], prev_state=[%0d, %0d, %0d, %0d, %0d, %0d]",
                        step_count+1, delay,signal_in,
                        survivor_path_metrics[0], survivor_path_metrics[1], survivor_path_metrics[2],
                        survivor_path_metrics[3], survivor_path_metrics[4], survivor_path_metrics[5],
                        prev_survivor_path_state[0], prev_survivor_path_state[1], prev_survivor_path_state[2],
                        prev_survivor_path_state[3], prev_survivor_path_state[4], prev_survivor_path_state[5]);
