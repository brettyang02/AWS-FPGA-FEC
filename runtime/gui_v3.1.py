# usage: sudo python3 gui_v2.8_light.py
import os
import mmap
import time
import struct
...
# Constants and FpgaDriver remain identical to your original code...
# (Omitted here for brevity, keep your original ADDR_ and FpgaDriver class)

# ==============================================================================
# 1. CONSTANTS & ADDRESS MAP (Keeping your original)
# ==============================================================================
ADDR_CTRL        = 0x500
ADDR_N_INT       = 0x504
ADDR_PROB_IDX    = 0x508
...
# ==============================================================================
# 4. GUI APPLICATION (Light Theme Version)
# ==============================================================================
# CHANGED: Appearance mode set to Light
...
class FpgaMonitorApp:
    def __init__(self, root, fpga):
        ...
        # ... (Your variables: is_sweeping, current_snr, etc. remain the same)
        ...
        self.mem_file = ""
        self.snr_ber_pre = {}; self.snr_ber_post = {}; self.snr_fer = {}
        self.table_col_idx = 1
        self.csv_data = []
        self.speed_calculated = False
        self.last_speed_time = 0; self.last_speed_bits = 0
    ...
    def _build_ui(self):
        ...
        for i, (label_text, var) in enumerate(metrics):
            ...
            # CHANGED: text_color set to gray40 for better light-mode contrast
            ctk.CTkLabel(grid_frame, text=label_text, font=("Roboto", 11, "bold"), text_color="gray30").grid(row=row_idx, column=col_off, sticky="w", pady=2, padx=(5, 2))
            ctk.CTkLabel(grid_frame, textvariable=var, font=("Consolas", 12), text_color="#2980b9").grid(row=row_idx, column=col_off+1, sticky="w", pady=2, padx=(0, 10))
        ...
        self.combo_pam = ctk.CTkOptionMenu(input_grid, values=["4", "6", "8"], width=60); self.combo_pam.set("6")
        ...
        self.entry_sep = ctk.CTkEntry(input_grid, width=60); self.entry_sep.insert(0, "24.0")
        ...
        # CHANGED: Status color to blue for light mode visibility
        ctk.CTkLabel(sweep_frame, textvariable=self.var_status, font=("Roboto", 11, "italic"), text_color="#2980b9").pack(pady=2)
        ...
        self.plot_pre_var = tk.BooleanVar(value=True); self.plot_post_var = tk.BooleanVar(value=True); self.plot_fer_var = tk.BooleanVar(value=True)
        ...
        # 4. Speed Indicator
        # CHANGED: Lighter background for light mode
        speed_frame = ctk.CTkFrame(left_frame, fg_color="#ebebeb", corner_radius=10)
        speed_frame.pack(fill="x", pady=(20, 0), padx=5)
        ctk.CTkLabel(speed_frame, text="Sim Speed:", font=("Roboto", 13, "bold"), text_color="gray30").pack(side="left", padx=15, pady=10)
        ctk.CTkLabel(speed_frame, textvariable=self.var_speed, font=("Consolas", 15, "bold"), text_color="#d35400").pack(side="right", padx=15, pady=10)
        ...
        chart_container = ctk.CTkFrame(graph_panel, corner_radius=15, fg_color="white") # Force white bg
        chart_container.pack(fill="both", expand=True)
        
        self.fig, self.ax = plt.subplots(figsize=(6, 4), dpi=100)
        
        # CHANGED: Matplotlib styling for light mode
        self.fig.patch.set_facecolor('white')
        self.ax.set_facecolor('white')
        [self.ax.spines[s].set_edgecolor('#cccccc') for s in self.ax.spines]
        self.ax.set_title("BER Waterfall Curve", color="black")
        self.ax.set_xlabel("SNR (dB)", color="black")
        self.ax.set_ylabel("Error Rate", color="black")
        self.ax.set_yscale('log')
        self.ax.tick_params(axis='both', which='both', colors='black')
        self.ax.grid(True, which="both", color='#eeeeee', linestyle='--', alpha=0.8)
        ...
        # CHANGED: Lighter table background
        self.table_frame = ctk.CTkScrollableFrame(graph_panel, height=180, corner_radius=15, fg_color="#f5f5f5", orientation="horizontal")
        ...
        for row, text in enumerate(headers):
            lbl = ctk.CTkLabel(self.table_frame, text=text, font=("Roboto", 13, "bold"), text_color="#2980b9", width=120, anchor="e")
        ...
        self.btn_save_graph = ctk.CTkButton(button_container, text="💾 Save Graph", fg_color="#34495e", command=self.save_graph_btn); self.btn_save_graph.pack(side="right", padx=(5, 0))
        self.btn_save_csv = ctk.CTkButton(button_container, text="💾 Save CSV", fg_color="#34495e", command=self.save_csv_btn); self.btn_save_csv.pack(side="right", padx=(5, 0))
    ...
    def update_plot_visibility(self):
        ...
        if visible_handles:
            # CHANGED: Legend colors for light mode
            self.ax.legend(visible_handles, visible_labels, facecolor='white', edgecolor='#cccccc', labelcolor='black')
        else:
    ...
    def add_table_col(self, snr, pre_ber, post_ber, fer, elapsed):
        ...
        col_data = [f"{snr:.1f} dB", f"{pre_ber:.2e}", f"{post_ber:.2e}", f"{fer:.2e}", f"{elapsed:.1f} s"]
        for row, text in enumerate(col_data):
            # CHANGED: Text color set to black for light mode visibility
            lbl = ctk.CTkLabel(self.table_frame, text=text, font=("Consolas", 12), text_color="black", width=90)
            lbl.grid(row=row, column=self.table_col_idx, padx=5, pady=2)
        self.table_col_idx += 1
        self.table_frame._parent_canvas.xview_moveto(1.0) 

    # ... (Rest of your methods: clear_table, save_graph_btn, etc. remain unchanged)
        ...
        if fpath:
            try:
                with open(fpath, 'w', newline='') as f:
                    ...
                    for row in self.csv_data: writer.writerow(row)
    ...
    def init_fpga(self):
        ...
        self.speed_calculated = False; self.last_speed_time = 0; self.last_speed_bits = 0
    ...
    def manual_reset_btn(self):
        try:
            self.current_pam = int(self.combo_pam.get()); self.current_sep = float(self.entry_sep.get())
            self.current_snr = float(self.sweep_entries["Start SNR:"].get())
        except ValueError: pass 
        self.snr_display_var.set(f"{self.current_snr:.1f} dB"); self.init_fpga()
        self.start_time = time.time(); self.snr_ber_pre.clear(); self.snr_ber_post.clear(); self.snr_fer.clear(); self.clear_table()
    ...
    def toggle_sweep(self):
        if not self.is_sweeping:
            try:
                ...
                self.target_stat = self.combo_target_stat.get(); self.is_sweeping = True
                self.clear_table(); self.btn_sweep.configure(text="Stop Sweep", fg_color="#c0392b"); self._transition_to_current_snr()
            except Exception as e: 
                self.var_status.set("Error: Invalid Inputs"); print(f"Sweep start error: {e}")
        else:
            self.is_sweeping = False; self.btn_sweep.configure(text="Start Sweep", fg_color="#27ae60")

    def _transition_to_current_snr(self):
        self.snr_display_var.set(f"{self.current_snr:.1f} dB"); self.init_fpga(); self.start_time = time.time()
    ...
    def poll_fpga(self):
        ...
        current_time = time.time(); elapsed_time = current_time - self.start_time
        if not self.speed_calculated and elapsed_time > 1.0:
            if self.last_speed_time == 0: self.last_speed_time = current_time; self.last_speed_bits = bits
            else:
                delta_time = current_time - self.last_speed_time; delta_bits = bits - self.last_speed_bits
                if delta_time > 0 and delta_bits > 0:
                    mbps = (delta_bits / delta_time) / 1_000_000.0
                    self.var_speed.set(f"{mbps:.2f} Mbps"); self.speed_calculated = True
        ...
        self.line_pre.set_data(snrs, [self.snr_ber_pre[s] for s in snrs]); self.line_post.set_data(snrs, [self.snr_ber_post[s] for s in snrs]); self.line_fer.set_data(snrs, [self.snr_fer[s] for s in snrs])
