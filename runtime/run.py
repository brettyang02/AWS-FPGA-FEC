# usage:  sudo python3 run.py noise16dB_sep24.mem
import os
import mmap
import time
import struct
...
def main():
    ...
    # --- HANDLE ARGUMENTS ---
    mem_file = "no_noise.mem" # Default
    if len(sys.argv) > 1:
        mem_file = sys.argv[1]
    else:
        print(f"ℹ️  No file specified. Using default: {mem_file}")
        print("   Usage: sudo python3 test_noise.py <filename.mem>")
    ...
    load_probabilities(fpga, mem_file)
