# usage: sudo python3 gui_v2.2.py
import os
import mmap
import time
import struct
...
def get_or_create_noise_file(target_snr_db, pam_order, symbol_separation):
    """Generates the noise file on-the-fly if it does not exist."""
    
    # 1. Format the filename
    if target_snr_db >= 99:
        filename = f"noise_zero_PAM{pam_order}_sep{int(symbol_separation)}.mem"
    else:
        # Changed to .1f to keep decimal precision (e.g., noise10.5dB_...)
...
class FpgaMonitorApp:
    def __init__(self, root, fpga):
        ...
        self.current_pam = 6      # Default PAM
        self.current_sep = 24.0   # Default Separation
        ...
        self.snr_display_var = tk.StringVar(value=f"{self.current_snr:.2f} dB")
        self.sweep_entries = {}
    ...
    def _build_ui(self):
        ...
        # ROW 0: PAM Order and Symbol Sep
        ...
        self.combo_target_stat.set("Pre-FEC Errors"); self.combo_target_stat.grid(row=3, column=1, columnspan=3, sticky="ew", pady=5)
        ...
        self.table_frame = ctk.CTkScrollableFrame(graph_panel, height=180, corner_radius=15, fg_color="#242424")
        self.table_frame.pack(fill="x", expand=False, pady=(15, 10))

        headers = ["Sweeped SNR", "Pre-FEC BER", "Post-FEC BER", "Time (s)"]
        for col, text in enumerate(headers):
            lbl = ctk.CTkLabel(self.table_frame, text=text, font=("Roboto", 13, "bold"), text_color="#3498db", width=130)
        ...
        self.btn_save_graph.pack(side="right", pady=(5, 0))

    def add_table_row(self, snr, pre_ber, post_ber, elapsed):
        row_data = [
            f"{snr:.2f} dB",
            f"{pre_ber:.2e}",
            f"{post_ber:.2e}",
        ...
        for col, text in enumerate(row_data):
            lbl = ctk.CTkLabel(self.table_frame, text=text, font=("Consolas", 12), text_color="white", width=130)
            lbl.grid(row=self.table_row_idx, column=col, padx=10, pady=2)
    ...
    def save_graph_btn(self):
        fpath = filedialog.asksaveasfilename(defaultextension=".png", initialfile=f"ber_PAM{self.current_pam}_{self.current_snr}dB.png", filetypes=[("PNG", "*.png"), ("PDF", "*.pdf")])
        if fpath:
            try:
    ...
    def manual_reset_btn(self):
        # Update current parameters from GUI inputs before reset
        try:
            ...
            pass # Keep previous valid values if input is malformed
            
        self.snr_display_var.set(f"{self.current_snr:.2f} dB")
        self.init_fpga()
        self.start_time = time.time()
        self.snr_ber_pre.clear(); self.snr_ber_post.clear(); self.clear_table()
    ...
    def poll_fpga(self):
        ...
        if self.is_sweeping:
            ...
            if val >= self.target_count:
                ...
                self.add_table_row(self.current_snr, b_pre, b_post, elapsed_time)
