# usage: sudo python3 gui_v2.5.py
import os
import mmap
import time
import struct
...
class FpgaMonitorApp:
    def __init__(self, root, fpga):
        ...
        # Track columns instead of rows for the transposed table
    ...
    def _build_ui(self):
        ...
        self.ax.set_ylabel("Bit Error Rate (BER)", color="white")
        self.ax.set_yscale('log')
        
        # This line forces ALL ticks (major and minor, x and y) to be white
    ...
    def add_table_col(self, snr, pre_ber, post_ber, elapsed):
        col_data = [
            f"{snr:.1f} dB",
            f"{pre_ber:.2e}",
            f"{post_ber:.2e}",
    ...
    def poll_fpga(self):
        ...
        # --- CALCULATE SIMULATION SPEED USING DELTA ---
        if not self.speed_calculated and elapsed_time > 1.0:
            if self.last_speed_time == 0:
                # Take a baseline snapshot after the hardware is fully running (at t = 1s)
                self.last_speed_time = current_time
                self.last_speed_bits = bits
            else:
                # Compare the next cycle (t = 2s) to the baseline
        ...
        if self.is_sweeping:
            ...
            if val >= self.target_count:
                ...
                self.add_table_col(self.current_snr, b_pre, b_post, elapsed_time)
