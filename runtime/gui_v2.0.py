...
# ==============================================================================
# 3. ON-THE-FLY PROBABILITY GENERATOR & LOADER 
# ==============================================================================
...
class FpgaMonitorApp:
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(stats_frame, text="System Metrics", font=("Roboto", 18, "bold")).pack(pady=10)

        self.snr_display_var = tk.StringVar(value=f"{self.current_snr} dB")
        metrics = [
            ("Current SNR:", self.snr_display_var),
            ...
            ("Total Frames:", self.var_total_frames),
            ("Frame Errors:", self.var_frame_errs),
            ("FER:", self.var_fer)
        ]
        ...
        for i, (label_text, var) in enumerate(metrics):
            ctk.CTkLabel(grid_frame, text=label_text, font=("Roboto", 12, "bold"), text_color="gray70").grid(row=i, column=0, sticky="w", pady=2, padx=10)
            ctk.CTkLabel(grid_frame, textvariable=var, font=("Consolas", 13), text_color="#3498db").grid(row=i, column=1, sticky="e", pady=2, padx=10)
            grid_frame.grid_columnconfigure(1, weight=1)

        ctk.CTkButton(stats_frame, text="Manual Reset Core", command=self.manual_reset_btn).pack(pady=15, padx=20, fill="x")
        ...
        entries = [("Start SNR:", "10"), ("Stop SNR:", "30"), ("Step Size:", "1")]
        self.sweep_entries = {}
        for i, (txt, dflt) in enumerate(entries):
            ctk.CTkLabel(input_grid, text=txt).grid(row=i, column=0, sticky="w", pady=2)
            e = ctk.CTkEntry(input_grid, width=100)
            e.insert(0, dflt)
            e.grid(row=i, column=1, sticky="e", pady=2)
            self.sweep_entries[txt] = e

        ctk.CTkLabel(input_grid, text="Stop Target:").grid(row=3, column=0, sticky="w", pady=2)
        self.combo_target_stat = ctk.CTkOptionMenu(input_grid, values=["Pre-FEC Errors", "Post-FEC Errors", "Frame Errors", "Total Frames"], width=120)
        self.combo_target_stat.set("Pre-FEC Errors")
        self.combo_target_stat.grid(row=3, column=1, sticky="e", pady=2)

        ctk.CTkLabel(input_grid, text="Target Count:").grid(row=4, column=0, sticky="w", pady=2)
        self.entry_target_count = ctk.CTkEntry(input_grid, width=100)
        self.entry_target_count.insert(0, "100000")
        self.entry_target_count.grid(row=4, column=1, sticky="e", pady=2)
    ...
    def init_fpga(self):
        ...
        if not os.path.exists(self.mem_file):
            generate_pam6_noise(self.current_snr, self.mem_file)
    ...
    def toggle_sweep(self):
        if not self.is_sweeping:
            ...
            self.var_status.set(f"Sweeping... Target: {self.target_stat}")
    ...
    def poll_fpga(self):
        ...
        # Read Counters
        bits     = self.fpga.read64(ADDR_BITS_LO, ADDR_BITS_HI)
        err_pre  = self.fpga.read64(ADDR_ERR_PRE_LO, ADDR_ERR_PRE_HI)
        err_post = self.fpga.read64(ADDR_ERR_POST_LO, ADDR_ERR_POST_HI)
        frames   = self.fpga.read64(ADDR_FRAMES_LO, ADDR_FRAMES_HI) #
        f_errs   = self.fpga.read64(ADDR_FRAME_ERR_LO, ADDR_FRAME_ERR_HI) #

        # Calculations
        ber_pre  = err_pre / bits if bits > 0 else 0.0
        ber_post = err_post / bits if bits > 0 else 0.0
        fer      = f_errs / frames if frames > 0 else 0.0

        # Update UI Labels
        self.var_time.set(f"{time.time() - self.start_time:.2f}")
        self.var_total_bits.set(f"{bits:,}")
        ...
        self.var_total_frames.set(f"{frames:,}") #
        self.var_frame_errs.set(f"{f_errs:,}") #
        self.var_fer.set(f"{fer:.2e}") #

        # Update Graph
        p_pre, p_post = max(ber_pre, 1e-12), max(ber_post, 1e-12)
        self.snr_ber_pre[self.current_snr], self.snr_ber_post[self.current_snr] = p_pre, p_post
        ...
        if self.is_sweeping:
            val = 0
            if self.target_stat == "Pre-FEC Errors": val = err_pre
            elif self.target_stat == "Post-FEC Errors": val = err_post
            elif self.target_stat == "Frame Errors": val = f_errs #
            elif self.target_stat == "Total Frames": val = frames #
...
def main():
    MOCK_MODE = True 
    if not MOCK_MODE and sys.platform != "win32" and os.getuid() != 0:
