...
class FpgaMonitorApp:
    def __init__(self, root, fpga):
        ...
        self.root.geometry("1200x900") # Slightly taller to accommodate the table
    ...
    def add_table_row(self, snr, pre_ber, post_ber, elapsed):
        ...
        self.table_row_idx += 1
        self.table_frame._parent_canvas.yview_moveto(1.0) # Auto-scroll to bottom

    def clear_table(self):
        for widget in self.table_frame.winfo_children():
            # Destroy widgets that aren't in the header row (row 0)
    ...
    def manual_reset_btn(self):
        self.init_fpga(); self.start_time = time.time(); self.snr_ber_pre.clear(); self.snr_ber_post.clear(); self.clear_table()
    ...
    def toggle_sweep(self):
        if not self.is_sweeping:
            try:
                ...
                self.clear_table() # Reset table for new sweep
