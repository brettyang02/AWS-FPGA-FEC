# usage: python gui_waterfall_auto_sep24.py
import os
import mmap
import time
import struct
...
def generate_pam6_noise(target_snr_db, filename, symbol_separation=24.0):
    ...
    print(f"\n--- Generating Configuration: {filename} ---")
    print(f"  > Modulation: PAM-6 | Separation: {symbol_separation} (d={d}) | Es: {Es:.2f}")
    ...
    if target_snr_db > 99:
        ...
        print(f"  > SNR: {target_snr_db} dB | Noise Sigma: {sigma:.4f}")
...
def load_probabilities(fpga, filename):
    print(f"-> Loading probabilities from {filename} into core...")
    try:
...
class FpgaMonitorApp:
    def __init__(self, root, fpga):
        ...
        self.root.title("PAM6 Autonomous BER Waterfall")
    ...
    def _build_ui(self):
        ...
        self.entry_start = ctk.CTkEntry(input_grid, width=120)
        ...
        self.entry_stop = ctk.CTkEntry(input_grid, width=120)
        ...
        self.entry_step = ctk.CTkEntry(input_grid, width=120)
        self.entry_step.insert(0, "1")
        self.entry_step.grid(row=2, column=1, sticky="e", pady=5)
        
        ctk.CTkLabel(input_grid, text="Stop Target:").grid(row=3, column=0, sticky="w", pady=5)
        self.combo_target_stat = ctk.CTkOptionMenu(input_grid, values=["Pre-FEC Errors", "Post-FEC Errors"], width=120)
        self.combo_target_stat.set("Pre-FEC Errors")
        self.combo_target_stat.grid(row=3, column=1, sticky="e", pady=5)

        ctk.CTkLabel(input_grid, text="Error Count:").grid(row=4, column=0, sticky="w", pady=5)
        self.entry_target_errors = ctk.CTkEntry(input_grid, width=120)
        self.entry_target_errors.insert(0, "100000")
        self.entry_target_errors.grid(row=4, column=1, sticky="e", pady=5)
    ...
    def init_fpga(self):
        ...
        # Check if the memory file exists; if not, generate it dynamically
        if not os.path.exists(self.mem_file):
            print(f"\n⚠️  Missing {self.mem_file}. Initiating on-the-fly generation...")
    ...
    def toggle_sweep(self):
        if not self.is_sweeping:
            try:
                ...
                self.target_errors = int(self.entry_target_errors.get())
            except ValueError:
            ...
            self.var_status.set(f"Sweeping... Target: {self.target_errors:,} {self.target_stat.split('-')[0]}")
    ...
    def poll_fpga(self):
        ...
        if self.is_sweeping:
            current_errors = err_pre if self.target_stat == "Pre-FEC Errors" else err_post
            
            if current_errors >= self.target_errors:
