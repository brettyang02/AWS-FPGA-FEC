...
class GrayDecodePAM6:
    ...
    def push_symbol(self, s: int) -> List[int]:
        ...
        if self.phaseI:
            ...
            #     bits = "00000"
            # new_bits = [int(c) for c in bits]

            # 1. Get current 5-bit string from table
            current_str = DECODE_TABLE.get((self._I, Q), "00000")
            current_bits = [int(b) for b in current_str]
            
            # 2. Match RTL: Output the OLD bits, then save the NEW bits
            out = self._prev_bits
            self._prev_bits = current_bits        
